...
export const getUserProfile = async (req, res) => {
  const {id} = req.params
  try {
    const user = await User.findById(id).select("-password").populate('referred_by');
...
export const getTransactions = async (req, res) => {
  try {
    ...
    const investments = await Investment.find({ user: id }).sort({ createdAt: -1 });
...
export const getCommission = async(req, res) => {
  ...
  try {
    const commissions = await Commission.find({ to_user: id, level: { $gt: 1 } }).populate("from_user")
